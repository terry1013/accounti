...
public class AccountI {
	...
	public static void main(String[] args) {
		...
		if (RUNNING_MODE.equals(ONE_TASK)) {
			...
			AccountI.logger.log(Level.INFO, "OneTask parameter found in .properties. file Task name = " + cln);
			try {
				...
				AccountI.logger.log(Level.SEVERE, e.getMessage(), e);
				Exit.shutdown();
			}
		}
	}
	...
	public static void saveProperty() {
		...
		if (Session.getUser() != null && actualContent == DOCKING) {
			...
			TPreferences.setPreference(TPreferences.WINDOW_STATE, AccountI.class.getName(), s);
			if (s != WebFrame.MAXIMIZED_BOTH) {
				Rectangle bo = frame.getBounds();
				TPreferences.setPreference(TPreferences.WINDOW_BOUND, AccountI.class.getName(), bo);
			}
		}
	}
	...
	public static void setContentPane(int p) {
		...
		if (p == DOCKING) {
			...
			// frame state
			boolean ani = true;
			int sts = (Integer) TPreferences.getPreference(TPreferences.WINDOW_STATE, AccountI.class.getName(),
					JFrame.NORMAL);
	...
	public static void showNotification(String mid, int lt, Object... dta) {
		...
		WebNotificationPopup npop = NotificationManager.showNotification(AccountI.frame, ae.getMessage(),
				ae.getExceptionIcon());
