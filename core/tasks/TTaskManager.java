...
class RequestMaximized implements Runnable {
	...
	public void run() {
		...
		if (rm != null && rm.equals("true")) {
			AccountI.frame.setState(JFrame.NORMAL);
...
class ExecuteTaskAndExit implements Runnable {
	...
	public ExecuteTaskAndExit(String tn) {
		this.taskN = tn;
		AccountI.logger.log(Level.INFO, "MVEL task " + taskN + " created.");
	...
	public void run() {
		try {
			...
			AccountI.logger.log(Level.INFO, "MVEL task " + taskN + " Ended.");
			Exit.shutdown();
		} catch (Exception e) {
			AccountI.logger.logp(Level.SEVERE, null, null, e.getMessage(), e);
			Exit.shutdown();
		}
	}
}
